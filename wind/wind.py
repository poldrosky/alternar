...
dictDirection = {'1':(345,15), '2':(15,45), '3':(45,75), '4':(75,105),
                '5':(105,135), '6':(135,165), '7':(165, 195), '8':(195, 225),
                '9':(225, 255), '10':(255, 285), '11':(285, 315), '12':(315, 345)}
...
dictDirection1 = {'1':'N', '2':'NNE', '3':'ENE', '4':'E',
              '5':'ESE', '6':'SSE', '7':'S', '8':'SSW',
              '9':'WSW', '10':'W', '11':'WNW', '12':'NNW'}
...
for station in stations:
    ...
    dataList = []
    timeWindBack = rows[0][3].date()
    dataList.append([])
    count = 0
    for row in rows:
        timeWindCurrent = row[3].date()
        if(timeWindCurrent != timeWindBack):
            dataList[count].append('\n')
            dataList.append([])
            count += 1
            timeWindBack = timeWindCurrent
        directionWind = classifyDirection(row[6],dictDirection)
        speedWind = classifySpeed(row[7],dictSpeed)
    ...
    db.copyToTable('patterns', io.StringIO(stdin))
